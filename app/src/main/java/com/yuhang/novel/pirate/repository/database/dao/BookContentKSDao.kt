...
interface BookContentKSDao {
     ...
     */
    @Insert
    suspend fun insert(obj: BookContentKSEntity)

    /**
     * 查找章节内容
     */
    @Query("select * from bookcontentksentity as c where c.chapterId = :chapterid and c.bookId = :bookid limit 1")
    suspend fun query(bookid: String, chapterid: String): BookContentKSEntity?

    /**
     * 查询章节内容
     */
    @Query("select * from bookcontentksentity as c where c.chapterId = :chapterid and c.bookId = :bookid limit 1")
    suspend fun queryObj(bookid: String, chapterid: String): BookContentKSEntity?

    /**
     * 获取书本所有章节内容
     */
    @Query("select * from bookcontentksentity where bookId = :bookid")
    suspend fun queryAll(bookid:String):List<BookContentKSEntity>
...
//    fun queryTimeAndChapterid(bookid: Long): List<BookContentKSEntity?>
    ...
    suspend fun updateLastOpenContent(bookid: String, chapterid: String, lastOpenTime: Long, lastContentPosition: Int)

    /**
     * 根据小说id查询最近阅读章节
     */
    @Query("select * from bookcontentksentity as c  where c.bookId = :bookid  order by c.lastOpenTime desc limit 1")
    suspend fun queryLastOpenChapter2(bookid: String): BookContentKSEntity?
    ...
    suspend fun queryLastOpenChapter(bookid: String): BookContentKSEntity?
    ...
    suspend fun delete(bookid: String, chapterid: String)
