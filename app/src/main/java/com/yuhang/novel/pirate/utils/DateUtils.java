...
public class DateUtils {
    ...
    public static String getTimeZhanxin(Date dt) {

        long oldTime = dt.getTime();
        long newTime = System.currentTimeMillis();

        long timeDifference = (newTime - oldTime) / 1000;

        if (timeDifference < 60) {  //秒
            if (timeDifference <= 10) {
                return "刚刚";
            } else {
                return String.format("%d秒前", timeDifference);
            }
        } else if (timeDifference < 60 * 60) {  //分钟
            return String.format("%d分前", timeDifference / 60);
        } else if (timeDifference < 60 * 60 * 24) {  //小时
            return String.format("%d小时前", timeDifference / (60 * 60));
        } else if (timeDifference < 60 * 60 * 24 * 30) {  //天
            return String.format("%d天前", timeDifference / (60 * 60 * 24));
        } else if (timeDifference < 60 * 60 * 24 * 30 * 12) {  //月
            return String.format("%d月前", timeDifference / (60 * 60 * 24 * 30));
        } else { //年
            return String.format("%d年前", timeDifference / (60 * 60 * 24 * 30 * 12));
        }
