...
import com.yuhang.novel.pirate.utils.UrlEncoderUtils;
...
public class AnalyzeUrl {
    ...
    private String analyzeHeader(String ruleUrl, Map<String, String> headerMapF) {
        ...
        if (matcher.find()) {
            ...
            try {
                Map<String, String> map = new Gson().fromJson(find, BookResouceConstant.INSTANCE.getMAP_STRING());
    ...
    private String replaceJs(String ruleUrl, String baseUrl, Integer searchPage, String searchKey) throws Exception {
        if (ruleUrl.contains("{{") && ruleUrl.contains("}}")) {
            ...
            Matcher expMatcher = BookResouceConstant.INSTANCE.getEXP_PATTERN().matcher(ruleUrl);
            while (expMatcher.find()) {
                jsEval = BookResouceConstant.INSTANCE.getSCRIPT_ENGINE().eval(expMatcher.group(1), simpleBindings);
    ...
    private void generateUrlPath(String ruleUrl) {
        url = StringUtils.getAbsoluteURL(baseUrl, ruleUrl);
